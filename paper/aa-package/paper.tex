...
\\
In this paper we focus on the time delay estimation and its relative error. To date, a sample of about $220$ GLQs is available\footnote{https://research.ast.cam.ac.uk/lensedquasars/index.html}. However, only a very small subset, consisting of the brightests and with with well separated multiple images, has been used to measure $H_0$. The reason relies on the fact that it is easier and safer to extract information, and consequently to reduce the error on $H_0$, for such systems.
\\
In this respect, Fig. \ref{fig:qsosepmagboth} shows the distribution of the multiple images versus the maximum image separation for the known GLQs. Systems being part of the grey region, which represent a consistent fraction of the total sample, can be regarded as not optimal candidates, since they are less bright and/or have very small image separations. 
\begin{figure}[h]
	\centering
